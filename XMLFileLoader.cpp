...
bool XMLFileLoader::mt_Set_File(const std::string& file_path)
{
	...
	if (m_verbose == true) log().mt_Entrance_Function(FUNCTION_NAME);
	...
	if (m_verbose == true) log().mt_Exit_Function();
	return l_b_ret;
}
...
void XMLFileLoader::mt_Work(void)
{
	...
	if (m_current_file_it != m_files.end())
	{
		...
		{
			log().mt_Entrance_Function("XML Loading");
			log() << "Loading " << m_current_file_it->first << '\n';
			m_verbose = true;
...
bool XMLFileLoader::mt_Recursive_Exploration(const TiXmlElement* current_element, LoadingStructure& loading_struct, XML_CallbackContainer& on_entry_callbacks, XML_CallbackContainer& on_exit_callbacks)
{
	...
	if (m_verbose == true) log() << "++ '" << l_path << "' ++ at line : " << current_element->Row() << '\n';
	...
	if (l_b_ret == true)
	{
		...
		if (m_verbose == true) log() << "-- '" << l_path << "' -- at line : " << l_previous_element->Row() << '\n';
